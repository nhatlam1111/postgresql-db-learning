...
Có nhiều phần mềm database khác nhau. Dưới đây là bức tranh tổng thể để bạn hình dung:

| Phần mềm | Giá | Đặc điểm nổi bật | Ai dùng nhiều? |
|---|---|---|---|
| **PostgreSQL** | Miễn phí | Mạnh nhất trong nhóm miễn phí, chuẩn SQL tốt nhất | Instagram, Grab, Shopee |
| MySQL | Miễn phí | Phổ biến nhất, đơn giản | Facebook, WordPress |
| SQL Server | Trả phí | Tích hợp tốt với Microsoft | Doanh nghiệp dùng Windows |
| SQLite | Miễn phí | Nhẹ, chạy trên điện thoại | Ứng dụng mobile |
| Oracle | Rất đắt | Dành cho tập đoàn lớn | Ngân hàng, viễn thông |

> 💡 **Tin vui:** SQL là ngôn ngữ chung — học SQL trên PostgreSQL, bạn có thể áp dụng 90% kiến thức cho bất kỳ database nào khác. Giống như biết lái xe số tự động rồi thì chuyển sang xe khác cũng dễ thôi.

Chúng ta chọn PostgreSQL vì:
...
### Lộ Trình 10 Tuần — Bạn Đang Ở Đây! 🗺️

Trước khi tóm tắt, hãy nhìn bức tranh tổng thể về hành trình bạn sắp đi qua:

```
Tuần 1    Tuần 2     Tuần 3     Tuần 4     Tuần 5     Tuần 6     Tuần 7     Tuần 8     Tuần 9     Tuần 10
  ↓         ↓          ↓          ↓          ↓          ↓          ↓          ↓          ↓          ↓
Tại sao → Cài đặt → Tạo bảng → SELECT  → Filter  → Pivot   → JOIN    → Nâng cao → Quản lý → Dự án
cần DB    PG+DB     INSERT     cơ bản     Sort      GROUP BY  VLOOKUP   CASE WHEN  UPDATE    thực tế
                                                              liên bảng  Subquery   DELETE    quán café
  ▲
  ║
  ╚══ BẠN ĐANG Ở ĐÂY
```

| Giai đoạn | Tuần | Bạn sẽ làm được gì? |
|---|---|---|
| **Nền tảng** | 1–2 | Hiểu tại sao cần database, cài đặt xong công cụ |
| **Xây dựng** | 3–4 | Tự tạo bảng, nhập dữ liệu, xem dữ liệu |
| **Phân tích** | 5–7 | Lọc, sắp xếp, tổng hợp, kết nối bảng — thay thế Excel |
| **Nâng cao** | 8–9 | Viết truy vấn phức tạp, quản lý dữ liệu an toàn |
| **Thực chiến** | 10 | Xây dựng hệ thống quản lý quán café từ đầu! |

---

### 3 Hiểu Lầm Phổ Biến Của Người Mới

Trước khi đi tiếp, hãy gỡ bỏ vài hiểu lầm thường gặp:

| Hiểu lầm | Sự thật |
|---|---|
| ❌ "Database sẽ thay thế Excel" | ✅ Không! Hai công cụ bổ trợ nhau. Nhiều BA dùng database để lấy dữ liệu, rồi phân tích trực quan trong Excel |
| ❌ "Phải giỏi lập trình mới học được SQL" | ✅ SQL gần với tiếng Anh hơn bất kỳ ngôn ngữ lập trình nào. Bạn không cần biết Python/Java |
| ❌ "Database chỉ dành cho IT/Developer" | ✅ BA, Data Analyst, kế toán, marketing — ai cần dữ liệu đều cần SQL. Đây là kỹ năng phân tích, không phải kỹ năng lập trình |
