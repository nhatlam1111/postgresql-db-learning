...
>
> 💡 **Lưu ý:** Mật khẩu đơn giản chỉ phù hợp cho môi trường học tập trên máy cá nhân. Trong công việc thực tế, luôn sử dụng mật khẩu mạnh.
...
> 💡 **Tại sao DBeaver mà không phải pgAdmin?**
> PostgreSQL đi kèm pgAdmin — nhưng DBeaver có giao diện đẹp hơn, dễ dùng hơn, và hỗ trợ nhiều loại database. Nếu sau này bạn làm việc với MySQL hay SQL Server, chỉ cần 1 công cụ DBeaver.
...
> 💡 **Phương án dự phòng:** Nếu gặp vấn đề khi cài đặt trên máy, bạn có thể thực hành SQL trực tuyến tại **db-fiddle.com** hoặc **sqliteonline.com** trong khi chờ khắc phục. Tuy nhiên, hãy cố gắng cài trên máy vì chúng ta sẽ cần PostgreSQL local cho các bài sau.
...
> 💡 **Schema là gì?** Khi mở rộng cây, bạn thấy Schemas → public. Schema giống như **thư mục con** bên trong database — giúp phân nhóm các bảng. Schema mặc định tên `public`, và tất cả bảng chúng ta tạo sẽ nằm ở đây. Hiện tại chỉ cần biết vậy.
