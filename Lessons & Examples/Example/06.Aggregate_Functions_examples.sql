...
-- Tạo bảng nhân viên (canonical schema — xem Reference/core_schema.md)
...
-- Tạo bảng đơn hàng (phiên bản rút gọn cho tuần 6 — khach_hang_id/ngay_giao sẽ thêm ở tuần 7)
...
INSERT INTO don_hang (ma_don_hang, nhan_vien_id, ngay_dat, tong_tien, trang_thai) VALUES
...
SELECT
    ...
    COUNT(DISTINCT dang_lam)      AS so_dang_lam
FROM nhan_vien;
...
FROM nhan_vien
WHERE dang_lam = TRUE    -- lọc hàng TRƯỚC
GROUP BY phong_ban
...
SELECT phong_ban, ROUND(AVG(luong), 0) AS luong_tb
FROM nhan_vien
WHERE dang_lam = TRUE    -- WHERE: lọc hàng trước
GROUP BY phong_ban
